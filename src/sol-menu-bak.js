import {getDefaults,rel2absIRI} from './utils.js';
import {fetchRdfData} from './rdf-utils.js';
import {menuCSS} from './view-menu-template.js';
...
export class SolMenu extends HTMLElement {
  ...
  async connectedCallback(){ 
    ...
    let data = await getMenuDataFromElement(this);
    createMenu(data,this);
  }
}
customElements.define("sol-menu",SolMenu);

async function getMenuDataFromElement(el){
...
async function getMenuDataFromRdf(uri){
  const data = [];
  ...
  async function createVerticalMenu( data, element ){
    let doc = element.ownerDocument;
    let win = doc.defaultView;
    let topUL = doc.createElement('UL')
    element.classList.add('sol-vertical-menu');
    for(var row of data){
      topUL.appendChild( await renderVerticalMenuItem(row,doc) )
    }
    element.appendChild(topUL);
  }

  async function renderVerticalMenuItem(row,doc){
    ...
    if(!row.parts){
      ...
      for(var m in row.parts){
        ...
        ul2.appendChild( await renderVerticalMenuItem(newItem,doc) )
      }
    }
    return li
  }
  async function createMenu( data, element ){
    let doc = element.ownerDocument;
    let win = doc.defaultView;
    ...
    for(var row of data){
      topUL.appendChild( await renderMenuItem(row,doc) )
    }
    element.appendChild(topUL);
  ...
  async function renderMenuItem(row,doc){
    const li = doc.createElement('LI')
    const span =  doc.createElement('SPAN')
    span.innerHTML = row.label
    li.appendChild(span)
    ...
    if(!row.parts){
      ...
      for(var m in row.parts){
        let newItem = row.parts[m]
        ul2.appendChild( await renderMenuItem(newItem,doc) )
      }
    }
    return li
  }

  const showMenuLink = (event,data)=>{
