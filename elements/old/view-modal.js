...
*/
  ...
  async render (o,solidUI) {
    ...
    modal.innerHTML = `
      <button style="${o.style}" title="${o.title}" onclick="window.sol.openModal(this)">
        ${o.label}
      </button>
      <div style="${s['.modal']}">
        <div class="suic-modal-content" style="${s['.modal-content']}">
          <div style="${s['.close']}" onclick="window.sol.closeModal(this)">
            &times;
          </div>
          ${o.content}
        </div>
  ...
  window.sol.openModal = function (element,action){
    element.parentElement.children[1].style.display = "block" ;
  }
  window.sol.closeModal = function (element,action){
