...
export class SolAutoComplete extends HTMLElement {
    ...
    async connectedCallback(){
        ...
        let defaultSelected = this.selected || this.getAttribute('selected');
        let data = await miniQuery(this,{wanted,limit});
        this.showAutocomplete(this,{ 
            options:data, 
            placeholder, 
            defaultSelected,
        });
    ...
    showAutocomplete(container, opts = {}) {
        ...
        let filtered = [];
        let activeIndex = -1;
        let defaultSelected = opts.defaultSelected;

        /* initial view shows selected */
        if(defaultSelected) {
          const selectedOption = options.find(option => option.value === defaultSelected);
          if(selectedOption) {
        ...
        function render(){
            ...
            filtered.forEach((opt, i) => {
                ...
                if(defaultSelected && defaultSelected == opt.value){
                    activeIndex=i;
        ...
        function updateFilter(reset){
            if(reset) filtered = options.slice();
...
return {
    ...
    } /* END OF FUNCTION showAutocomplete() */

} /* END OF CLASS SolAutoComplete */

customElements.define("sol-autocomplete",SolAutoComplete);

/*
  CREATE THE COMPONENT FROM A SCRIPT USING insertAutoComplete()
*/
