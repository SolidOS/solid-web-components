...
describe('SolQuery._assertSafeQuery', () => {
  ...
  UNSAFE.forEach(([q, kw]) => {
    test(`blocks ${kw}`, () => {
      expect(() => SolQuery._assertSafeQuery(q)).toThrow(new RegExp(kw, 'i'));
    });
  });

  test('case-insensitive match (lowercase drop)', () => {
    expect(() => SolQuery._assertSafeQuery('drop graph <g>')).toThrow(/DROP/);
  });
...
describe('SolQuery._sanitizeVarValue', () => {
  test('passes plain string through', () => {
    expect(SolQuery._sanitizeVarValue('alice')).toBe('alice');
  });
  ...
  test('rejects "{"', () => {
    expect(() => SolQuery._sanitizeVarValue('foo{bar}')).toThrow(/\{ \}/);
  });

  test('rejects "}"', () => {
    expect(() => SolQuery._sanitizeVarValue('bar}')).toThrow(/\{ \}/);
  });

  test.each(['INSERT', 'DELETE', 'DROP', 'CREATE', 'CLEAR', 'LOAD', 'COPY', 'MOVE', 'ADD'])(
    'rejects value containing %s', (kw) => {
      expect(() => SolQuery._sanitizeVarValue(`xx ${kw} yy`)).toThrow(/blocked keyword/);
    });

  test('rejects keyword case-insensitively', () => {
    expect(() => SolQuery._sanitizeVarValue('plz drop tables')).toThrow(/blocked keyword/);
  });
  ...
  test('allows substrings of keywords (e.g. "dropped" → contains DROP at start)', () => {
    ...
    expect(() => SolQuery._sanitizeVarValue('dropped')).not.toThrow();
  });
});
