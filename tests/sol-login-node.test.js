...
import { SolidAuth } from '../sol-login-node.js';
...
describe('_baseDomain', () => {
  ...
  test('extracts last two parts', () => {
    expect(auth._baseDomain('pod.solidcommunity.net')).toBe('solidcommunity.net');
  });

  test('returns as-is for two parts', () => {
    expect(auth._baseDomain('example.com')).toBe('example.com');
  });

  test('returns as-is for single part', () => {
    expect(auth._baseDomain('localhost')).toBe('localhost');
  });
});
