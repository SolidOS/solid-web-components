...
import { sanitizeHtml, queryHtmlWithSelector } from '../shared/utils.js';
import {
  parseAcl,
  authsToRoleModel,
  roleModelToTurtle,
...
describe('SPARQL injection — _assertSafeQuery advanced', () => {
  test('blocks case variations (MiXeD cAsE)', () => {
    expect(() => SolQuery._assertSafeQuery('iNsErT DATA { <a> <b> <c> }')).toThrow(/INSERT/i);
    expect(() => SolQuery._assertSafeQuery('DrOp GRAPH <g>')).toThrow(/DROP/i);
    expect(() => SolQuery._assertSafeQuery('CLeaR ALL')).toThrow(/CLEAR/i);
  });

  test('blocks leading whitespace and newlines before keyword', () => {
    expect(() => SolQuery._assertSafeQuery('  \n INSERT DATA { <a> <b> <c> }')).toThrow(/INSERT/i);
    expect(() => SolQuery._assertSafeQuery('\t\nDROP GRAPH <g>')).toThrow(/DROP/i);
  });

  test('blocks keyword after comment-like prefix', () => {
    expect(() => SolQuery._assertSafeQuery('SELECT * WHERE {} INSERT DATA { <a> <b> <c> }')).toThrow(/INSERT/i);
  });
  ...
  test('blocks keyword embedded in multiline query', () => {
    ...
    DELETE WHERE { ?a ?b ?c }`;
    expect(() => SolQuery._assertSafeQuery(q)).toThrow(/DELETE/i);
  });

  test('allows SELECT with subquery', () => {
    const q = 'SELECT ?x WHERE { { SELECT ?x WHERE { ?x a foaf:Person } } }';
    expect(() => SolQuery._assertSafeQuery(q)).not.toThrow();
  });
  ...
  test('allows DESCRIBE queries', () => {
    expect(() => SolQuery._assertSafeQuery('DESCRIBE <http://example.org/alice>')).not.toThrow();
  });

  test('allows ASK queries', () => {
    expect(() => SolQuery._assertSafeQuery('ASK { <http://ex/a> ?p ?o }')).not.toThrow();
  });
  ...
  test('blocks LOAD INTO', () => {
    expect(() => SolQuery._assertSafeQuery('LOAD <http://ex/data> INTO <g>')).toThrow(/LOAD/i);
  });
});
...
describe('SPARQL injection — _sanitizeVarValue advanced', () => {
  test('rejects semicolons that could close a triple', () => {
    ...
    const result = SolQuery._sanitizeVarValue('value;with;semis');
  ...
  test('rejects angle brackets in value to prevent IRI injection', () => {
    ...
    const result = SolQuery._sanitizeVarValue('<http://evil.example>');
    expect(result).toBe('<http://evil.example>');
  });

  test('escapes backtick correctly', () => {
    const result = SolQuery._sanitizeVarValue('back`tick');
  ...
  test('rejects newlines used to break string context', () => {
    ...
    const result = SolQuery._sanitizeVarValue('line1\nline2');
    expect(result).toContain('line1');
    expect(result).toContain('line2');
  });

  test('rejects value that is exactly a blocked keyword', () => {
    expect(() => SolQuery._sanitizeVarValue('DROP')).toThrow(/blocked keyword/);
    expect(() => SolQuery._sanitizeVarValue('INSERT')).toThrow(/blocked keyword/);
    expect(() => SolQuery._sanitizeVarValue('DELETE')).toThrow(/blocked keyword/);
  });

  test('allows DROPDOWN (keyword is only partial match)', () => {
    expect(() => SolQuery._sanitizeVarValue('DROPDOWN')).not.toThrow();
  });

  test('allows LOADING (keyword is only partial match)', () => {
    expect(() => SolQuery._sanitizeVarValue('LOADING')).not.toThrow();
  });

  test('escapes triple-quote attempt', () => {
    const result = SolQuery._sanitizeVarValue('"""injected"""');
    expect(result).toBe('\\"\\"\\"injected\\"\\"\\"');
    expect(result).not.toContain('"""');
  });

  test('handles empty string', () => {
    expect(SolQuery._sanitizeVarValue('')).toBe('');
  });

  test('handles unicode text', () => {
    expect(SolQuery._sanitizeVarValue('\u00e9l\u00e8ve')).toBe('\u00e9l\u00e8ve');
  });

  test('rejects combined braces with keyword', () => {
    expect(() => SolQuery._sanitizeVarValue('{ DROP }')).toThrow();
  });
});
